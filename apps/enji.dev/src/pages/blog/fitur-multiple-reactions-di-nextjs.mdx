...
Pas user klik reaction, kan muncul animasi tuh, pas event `onClick` cuma push yang nilai animasi yang `x` sama `y` random. Nilai animasi tadi dirender jadi componentnya framer motion.
