...
<TIL.Items date="2023-12-26">

  <TIL.Item>
    
    ## The Surprising Equality: N.999... = N

    <TIL.ItemTags>
      <TIL.ItemTag>mathematics</TIL.ItemTag>
      <TIL.ItemTag>infinite-series</TIL.ItemTag>
      <TIL.ItemTag>number-theory</TIL.ItemTag>
    </TIL.ItemTags>

    let's consider the decimal `N.999...` where the `9`s go on forever. At first glance, it may appear as if it's infinitesimally close to `N`, but is it really equal?

    Proof by Algebraic Manipulation:
    To demonstrate this equality, we can employ a straightforward algebraic approach. Let x = 0.999... hence
    ...
    [View on Wikipedia](https://en.wikipedia.org/wiki/0.999...)
